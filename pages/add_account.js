...
import { new_account } from "../reducers/user";

export default function Add_account (props) {
    ...
    const [color, set_color] = React.useState('red');
    const [user_type, set_user_type] = React.useState('master_boy');
    ...
    const onChangeType = (type) => {
        ...
        let colord = type === 'member' ? 'rgb(201 17 201)' : 
        type === 'admin'? 'blue' : 
        type === 'super_admin' ? 'green' : 
        type === 'master_boy' ? 'red' :
        type === 'master_girl' ? '#ff45ac' : 'rgb(201 17 201)'
       ...
       const add_new_account = () =>{
        ...
        if(name.toLowerCase() !== 'master'){
            let body = {
             ...
             if(name && password){
                dispatch(new_account({body , set_error, set_success }))
             }else{
              set_error({msg: 'الرجاء ادخال الاسم و كلمة مرور الغرفة'})
             }
    ...
    return(
        ...
        <Text variant="titleLarge"  style ={{textAlign: 'center', flex: 2}}> اضافة حساب </Text>
       </View>              
...
style ={{textAlign: 'center', color: 'green', paddingVertical: 10, borderWidth: 1 ,
 borderColor: 'green', }}>
    {success}
          ...
          <Button mode='elevated' onPress={add_new_account }
                  labelStyle={{color: 'black'}}
                  style ={{width: 180 , height: 40 , marginVertical: 40 , alignSelf: 'center' , 
                 backgroundColor: '#fbeb58' , shadowOpacity: 0}} > اضافة الحساب </Button>
